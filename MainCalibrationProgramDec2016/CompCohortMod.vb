            ...
            For Each CWTRow In CWTList
                ...
                If CWTRow.cLookUp = DictionaryKey Then
                    ...
                    If TermFlag(Fish, TStep) = 1 Or Fish = 74 Then
                        TotMortTerm(Age, TStep) = TotMortTerm(Age, TStep) + CWTRow.cCatch * ExpFact * (1 + OtherMort(Fish, TStep)) + CNR(STk, Age, Fish, TStep)
                    Else
                        TotMortPterm(Age, TStep) = TotMortPterm(Age, TStep) + CWTRow.cCatch * ExpFact * (1 + OtherMort(Fish, TStep)) + CNR(STk, Age, Fish, TStep)
                    End If
                End If
            Next
            For Age = 2 To MaxAge
