...
Public Class mainform
    ...
    Public Sub btnExit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnRun.Click
        ...
        ReDim CNRFlag(1, 100, NumFish, NumSteps + 1)
        ReDim CNRInput(1, 100, 4, NumFish, NumSteps + 1)
        ...
        For irow = 0 To dsSupportData.Tables("NonRetention").Rows.Count - 1
            ...
            Yr = dsSupportData.Tables("NonRetention").Rows(irow)("Year") - 1980
        ...
        For irow = 0 To dsSupportData.Tables("ImputeRecoveries").Rows.Count - 1
            ...
            ImputeItem.cSourceFish = dsSupportData.Tables("ImputeRecoveries").Rows(irow)("SourceFishery")
            ImputeItem.cSourceTStep = dsSupportData.Tables("ImputeRecoveries").Rows(irow)("SourceTimeStep")
            ImputeItem.cImputedFish = dsSupportData.Tables("ImputeRecoveries").Rows(irow)("ImputedFishery")
            ImputeItem.cImputedTStep = dsSupportData.Tables("ImputeRecoveries").Rows(irow)("ImputedTimeStep")
...
secondpass:
        ...
        Dim testfile3 As String
        testfile3 = "C:\data\calibration\07Qbasic\testfile3"
        FileOpen(11, testfile3, OpenMode.Output)
        ...
        If Firstpass = True Then
            ...
            For Each pair In TheDictionary ' deals with each unique stock, broodyear, stage (fing/year) combination
                ...
                'COMPUTE ADJUSTMENT FACTORS FOR RECOVERIES
                adjcatch()

                'COMPUTE TOTAL CATCH IN EACH FISHERY AFTER ADJUSTMENT
                ' OF RECOVERIES
                Addcatch()
                ...
                Else
                    ...
                    MaturationRate()
                    'Commpute Exploitation Rates in Recovery Year
                    ExploitationRate()
                    Forward()
        ...
        If Firstpass = True Then
            ...
            'save merged out of base recoveries to CWTOOB
            'Try
            dbConnection2 = New OleDbConnection(SConStr)
            'Open the Connection 
            dbConnection2.Open()
        ...
        Else
            ...
            For STk = 1 To NumStk
                For Age = 2 To MaxAge
                    BaseCohortTable.Rows.Add(BasePeriodID, STk, Age, FormatNumber(StartCohort(STk, Age), 0))
