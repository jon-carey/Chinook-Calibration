...
Module MergeMod
    Public Sub Merge()
        ...
        'Run this merge with FRE.cmd after deleting first 4 rows in fre78ch.edt
        ...
        '    objdatacommand = New OleDbCommand("SELECT TimeStepName FROM TimeStep Where TimeStepID = " & TStep, objConnection)
        '    Dim ds As OleDbDataReader = objdatacommand.ExecuteReader()
        '    If ds.HasRows Then
        '        While ds.Read
        '            MonthNames(TStep) = ds.Item("TimeStepName")
        '        End While
        '    End If
        '    ds.Close()
        ...
        ''SET UP FISHERY NAMES

        'For Fish = 1 To NumFish + 1
        '    objdatacommand = New OleDbCommand("SELECT FisheryName FROM Fishery Where FisheryID = " & Fish, objConnection)
        '    Dim ds As OleDbDataReader = objdatacommand.ExecuteReader()
        '    If ds.HasRows Then
        '        While ds.Read
        '            FishName(Fish) = ds.Item("FisheryName")
        '        End While
        '    End If
        '    ds.Close()
        'Next
